       ...
       c: [to,                                                 # destination
           {class: :content,
            c: [link,                                          # title
               ...
               ]}.update(color ? {style: "background: repeating-linear-gradient(300deg, #{color}, #{color} #{env[:gradientA] ||= (rand(16) + 1) / 16.0}em, #000 #{env[:gradientA]}em, #000 #{env[:gradientB] ||= (rand(16) + 1) / 16.0}em); border-color: #{color}"} : {}),
