        ...
        scan_node = -> node, depth = 0 {
          ...
          if node.text? && node.inner_text.match?(/^[\n\t\s]+$/)
            ...
            if node.child
              ...
              # no recursion into opaque[0] or in-too-deep[1] nodes. emit them as opaque HTML literals

              # [0] so far the only opaque node is SVG. if you're reading a source doc for D3 there might be data want to convert to RDF inside the SVG,
              # in which case you can tweak the definition above.

              # [1] we haven't encountered depth issues in our reader on any of the gnarly stuff the web has for it to gnaw on,
              # but some RDF writers hit default stack size limits with ruby indirection/generics magic resulting in ~8 frames per node synergizing with pathologically-deep autogenerated doc structures
              # you can bump up the stack size but who's gonna know you can even do that or remember the env-var without looking it up?

              if OpaqueNode.member?(name) || depth > 32
                yield subject, Content, RDF::Literal(node.to_html, datatype: RDF.HTML)
              elsif child = scan_node[node.child, depth + 1]
