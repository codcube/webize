        ...
        scan_node = -> node, depth = 0 {
          ...
          if node.text? && node.inner_text.match?(/^[\n\t\s]+$/)
            ...
            if node.child
              ...
              # [0] so far the only opaque node is SVG. if you're reading a source doc for D3 there might be data you want to convert to RDF inside SVG,
              ...
              if OpaqueNode.member?(name) || depth >= MaxDepth
