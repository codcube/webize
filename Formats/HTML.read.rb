        ...
        scan_node = -> node, depth = 0 {
          ...
          if node.text? && node.inner_text.match?(/^[\n\t\s]+$/)
            ...
            if node.child
              ...
              # no recursion on opaque[0] or too-deep[1] nodes - emit as opaque HTML literals

              # [0] so far the only opaque node is SVG. if you're reading a source doc for D3 or even more exotic, have
              # SVG that is also RDF/XML or RDFa there might be data you want to read as RDF inside SVG: adjust OpaqueNodes constant
              ...
              if OpaqueNode.member?(name) || depth + node.children.size >= MaxDepth
            ...
            if node.next_sibling
              if sibling = scan_node[node.next_sibling, depth + 1]
